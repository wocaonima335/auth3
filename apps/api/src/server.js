...
const { parseJsonBody, sendJson, setCors, fileExists } = require('../../../packages/shared-utils');
...
function getRequestedUser(req, authContext) {
  if (authContext.role === 'admin') {
...
async function handleApiRequest(req, res, requestUrl) {
  ...
  if (req.method === 'POST' && pathname === '/api/v1/phase3/jobs') {
    ...
    if (queue.isEnabled()) {
      try {
        ...
        jobRepository.appendEvent(created.id, {
          level: 'info',
          message: '任务已写入 Redis pending 队列'
        });
      } catch (error) {
        jobRepository.markFailed(created.id, new Error(`Redis enqueue failed: ${error.message}`));
        sendJson(res, 503, {
          error: 'QUEUE_ENQUEUE_FAILED',
          message: `Redis 入队失败: ${error.message}`
        });
        return;
      }
    }
  ...
  if (req.method === 'POST' && retryMatch) {
    ...
    const retryJob = jobRepository.createRetryJob(jobId, getRequestedUser(req, authContext));
    if (queue.isEnabled()) {
      try {
        await queue.enqueue(retryJob.id);
        jobRepository.appendEvent(retryJob.id, {
          level: 'info',
          message: '重试任务已写入 Redis pending 队列'
        });
      } catch (error) {
        jobRepository.markFailed(retryJob.id, new Error(`Redis retry enqueue failed: ${error.message}`));
        sendJson(res, 503, {
          error: 'QUEUE_RETRY_ENQUEUE_FAILED',
          message: `Redis 重试入队失败: ${error.message}`
        });
        return;
      }
    }
  ...
  if (req.method === 'POST' && cancelMatch) {
    ...
    const current = jobRepository.getJob(jobId);
    if (!authorizeJobAccess(res, authContext, current)) {
      return;
    }
    ...
    if (terminalStatuses.includes(current.status)) {
      sendJson(res, 409, {
        ...
        message: `任务状态为 ${current.status}，无法取消`,
        job: summarizeJob(current)
      });
      return;
    }
    const canceled = jobRepository.markCanceled(jobId);
    if (queue.isEnabled() && current.status === 'queued') {
      try {
        await queue.removePending(jobId);
        jobRepository.appendEvent(jobId, {
          level: 'info',
          message: '任务已从 Redis pending 队列移除'
        });
      } catch (error) {
        jobRepository.appendEvent(jobId, {
          level: 'warn',
          message: `Redis pending 移除失败: ${error.message}`
        });
      }
    }
    sendJson(res, 200, {
